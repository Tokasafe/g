<div>
    @include('toast.toast')
    <!-- The button to open modal -->
    <label for="my_modal_6" class="btn btn-xs btn-square btn-info">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
            <path d="M10.75 4.75a.75.75 0 00-1.5 0v4.5h-4.5a.75.75 0 000 1.5h4.5v4.5a.75.75 0 001.5 0v-4.5h4.5a.75.75 0 000-1.5h-4.5v-4.5z" />
          </svg>
          
    </label>

    <!-- Put this part before </body> tag -->
    <input type="checkbox" id="my_modal_6" class="modal-toggle" />
    <div class="modal">
        <div class="modal-box">
            <div class="divider divider-success">Workflow Template</div>
           <form wire:submit.prevent='store' action="">
            <div class="w-full max-w-xs form-control">

                <x-input-label-req :value="__('Name')" />
                <input type="text" placeholder="Type here" wire:model='name'
                    class=" @error('name') border-rose-500 border-2 @enderror w-full max-w-xs input input-bordered input-success input-xs focus:outline-none focus:border-success focus:ring-success focus:ring-1" />
                <x-input-error :messages="$errors->get('name')" class="mt-0" />
            </div>
               <div class="modal-action">
                   <button type="submit" for="my_modal_6" class="btn btn-xs btn-success">save!</button>
                   <label for="my_modal_6" class="btn btn-xs btn-error">Close!</label>
                </div>
            </form>
        </div>
    </div>
</div>
