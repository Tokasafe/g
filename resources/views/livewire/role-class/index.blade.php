<div>
    ...
    <div class="mx-4 mt-4 overflow-x-auto rounded-md shadow-md">
        <table class="table table-xs">
            ...
            <tbody>
                @forelse ($RoleClass as $index => $item)
                    <tr class="text-center">
                        <th>{{ $RoleClass->firstItem() + $index }}</th>
                        <td>{{ $item->name }}</td>
                        <td>{{ $item->description }}</td>
                        <td>
                            <div class="flex flex-row justify-center gap-1">

                                <label wire:click="update_RoleClass({{ $item->id }})"
                                    class="btn btn-xs btn-warning ">Edit</label>
                                <label for="delete_data" wire:click="deleteRoleClass({{ $item->id }})"
    ...
    <div>{{ $RoleClass->links() }}</div>
    @livewire('role-class.update')
