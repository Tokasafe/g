<div>
    ...
    <div class="modal ">
        <div class="modal-box">
            <h3 class="text-lg font-bold text-center">Add WorkflowStep!</h3>
            <form wire:submit.prevent='store'>
                ...
                <div class="w-full max-w-xs form-control">
                    ...
                    <select wire:model='event_type'
                        class=" @error('event_type') border-rose-500 border-2 @enderror select select-success w-full select-xs max-w-xs focus:outline-none focus:border-success focus:ring-success focus:ring-1">
                        <option selected>{{ __('select') }} {{ __('event_type') }}</option>
                        @foreach ($EventType as $item)
                            @if (old('event_type') == $item->id)
