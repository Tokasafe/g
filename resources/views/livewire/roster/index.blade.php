<div>
    ...
    <div class="flex justify-between mx-4">
        <div>
            @livewire('roster.create')
        </div>
        <div class=" md:join">



            <input wire:model='searchRoster'
        ...
        <table class="table table-xs">
            ...
            <tbody>
                @forelse ($Roster as $index => $item)
                    <tr class="text-center">
                        <th>{{ $Roster->firstItem() + $index }}</th>
                        <td>{{ $item->name }}</td>
                        <td>
                            <div class="flex flex-row justify-center gap-1">

                                <label wire:click="update_Roster({{ $item->id }})"
                                    class="btn btn-xs btn-warning ">Edit</label>
                                <label for="delete_data" wire:click="deleteRoster({{ $item->id }})"
