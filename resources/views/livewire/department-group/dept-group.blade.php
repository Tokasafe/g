<div class="flex justify-between mx-4">
    <div>
        @livewire('department-group.create')
    </div>

    <div class=" md:join">

        <select wire:model='searchGroup'
        ...
        <input wire:model='searchDepartment '
            ...
            placeholder="Search Department..." type="text" />
...
<div class="mx-4 mt-4 overflow-x-auto rounded-t-lg rounded-b-lg shadow-md">
    ...
    <table class="table table-xs">
        ...
        <tbody>
            @forelse ($DeptGroup as $index => $item)
                <tr class="text-center">
                    <th>{{ $DeptGroup->firstItem() + $index }}</th>
                    <td>{{ $item->Group->name }}</td>
                    <td>{{ $item->Department->name }}</td>
                    <td>
                        <div class="flex flex-row justify-center gap-1">

                            <label wire:click="update_DepartmentGroup({{ $item->id }})"
                                class="btn btn-xs btn-warning ">Edit</label>
                            <label for="delete_data" wire:click="deleteDepartmentGroup({{ $item->id }})"
...
<div>{{ $DeptGroup->links() }}</div>
@livewire('department-group.update')
...
<div class="modal">
    <div class="modal-box">
        ...
        <form wire:submit.prevent='deleteFileDeptGroup'>
            <div class="mx-4 mt-4 overflow-x-auto rounded-t-lg rounded-b-lg shadow-md">
                <table class="table table-xs">
                    <thead class="bg-emerald-300">
                        <tr class="text-center">
